...
searchButton.addEventListener('click', async () => {
  ...
  let searched = document.getElementById('search') as HTMLInputElement;
  let query = searched.value
  let listaDeFilmes = await procurarFilme(query);
  let ul = document.createElement('ul');
  ul.id = "lista"

  let movies=new Array();
  movies=Object(listaDeFilmes).results;
...
function preencherLogin() {
  const inputUsername =  document.getElementById('login') as HTMLInputElement;
